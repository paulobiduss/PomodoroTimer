...
- Ao concluir todos os blocos:
  ...
  - overlay final mostra resumo comparativo de foco, sem depender de acumulado simples.
...
## 8. Reforma do Historico de Foco Diario (12/05/2026)

### Objetivo funcional
- Substituir o contador acumulado simples por historico de foco por data.
- Exibir uma linha compacta com foco de hoje, diferenca direta contra ontem e total semanal.
- Preservar o valor legado de `history/focus_minutes_today` migrando-o uma unica vez para o dia atual.

### Mudancas de arquitetura
- **`core/focus_history.py`**:
  - concentra registro diario de foco, leitura por data ISO, comparacao com ontem e soma semanal de segunda a domingo.
- **`core/settings.py`**:
  - expoe `focus_history` e `record_focus_session`, mantendo chaves antigas apenas para compatibilidade e migracao.
- **`ui/windows/timer_window.py`**:
  - registra apenas blocos de foco concluidos e exibe o resumo compacto.
- **`ui/windows/overlay_window.py`**:
  - usa o texto comparativo recebido no modo de conclusao, sem mensagem de acumulado.

### Validacao tecnica registrada
- `py -m unittest discover tests` executado com 6 testes passando.
- `py -m py_compile core\settings.py core\focus_history.py ui\windows\timer_window.py ui\windows\overlay_window.py main.py tests\test_focus_history.py` sem erros.
- Startup via `py main.py` confirmado por 4 segundos sem falha imediata; permaneceu apenas o warning conhecido de geometria do Windows.

## 9. Melhoria do Build Portatil (12/05/2026)

### Objetivo funcional
- Tornar `build.bat` mais confiavel e claro para gerar o pacote distribuivel.
- Manter `dist` e `work` fora do Google Drive em `C:\tmp`, evitando o bloqueio conhecido na etapa `COLLECT`.
- Produzir tambem um `.zip` portatil pronto para upload ou compartilhamento.

### Mudancas aplicadas
- Preflight para validar raiz do projeto, `main.py`, `assets\icon.png`, Python e PyInstaller.
- Selecao automatica entre `py` e `python`.
- Limpeza controlada de `C:\tmp\PomodoroTimer_dist`, `C:\tmp\PomodoroTimer_build` e `C:\tmp\PomodoroTimer_release`.
- `--specpath build\spec` para evitar `PomodoroTimer.spec` solto na raiz.
- Caminhos absolutos para `--icon` e `--add-data`, preservando o destino interno `assets`.
- Copia de `README.md`, `LICENSE` e `CHANGELOG.md` para a pasta final.
- Geracao de `C:\tmp\PomodoroTimer_release\PomodoroTimer-portable.zip`.

### Validacao tecnica registrada
- `cmd /c build.bat` concluido com sucesso.
- Executavel final confirmado em `C:\tmp\PomodoroTimer_dist\PomodoroTimer\PomodoroTimer.exe`.
- Pacote portatil confirmado em `C:\tmp\PomodoroTimer_release\PomodoroTimer-portable.zip`.
- `PomodoroTimer.spec` nao ficou na raiz do projeto.
- Executavel gerado iniciado por 5 segundos sem erro em `stderr`.
